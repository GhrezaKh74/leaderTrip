// منتقل‌شده از نسخهٔ اول (`src/lib/`) بدون تغییر.
//
// تکرار عمدی و موقتی است: این دو فایل در نسخهٔ اول تست شده‌اند و کار می‌کنند،
// و ساختن یک بستهٔ مشترک برای دو اپی که یکی‌شان قرار است بازنشسته شود، هزینهٔ
// بی‌جهت است. با حذف نسخهٔ اول در فاز ف‌۳، این نسخه تنها نسخه می‌شود.
...
function jalCal(jy: number): JalaliCal {
  ...
  const gy = jy + 621
  let leapJ = -14
  // `breaks` آرایهٔ ثابتی است که همیشه پر است، ولی با `noUncheckedIndexedAccess`
  // کامپایلر این را نمی‌داند. به‌جای خاموش‌کردن قاعده، نبودِ مقدار همان‌جا خطا
  // می‌شود — چون اگر روزی واقعاً خالی باشد، سکوت بدترین رفتار است.
  const first = breaks[0]
  const last = breaks[bl - 1]

  if (first === undefined || last === undefined) throw new Error('جدول سال‌های کبیسه خراب است')

  let jp = first

  if (jy < jp || jy >= last) throw new Error('سال شمسی خارج از محدودهٔ پشتیبانی')
...
export function toFa(v: string | number): string {
  return String(v).replace(/\d/g, (d) => '۰۱۲۳۴۵۶۷۸۹'[Number(d)] ?? d)
}
...
export function fromISODate(iso: string): Date {
  ...
  // تاریخ نامعتبر به‌جای پرتاب خطا، `Invalid Date` می‌شود: فراخوان با
  // `Number.isNaN(date.getTime())` تصمیم می‌گیرد، و یک فیلد نیمه‌تایپ‌شده
  // نباید کل صفحه را پایین بیاورد.
