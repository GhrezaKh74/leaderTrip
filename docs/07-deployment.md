...
دو راه استقرار هست؛ اول تصمیم بگیرید کدامید:

| | چه وقتی | چطور |
|---|---|---|
| **الف. سرور با ترمینال/SSH** | سرور مجازی یا هاست داکری که compose دارد | گام‌به‌گام همین سند (توصیه‌شده) |
| **ب. پنل تک‌Dockerfile** | پنلی که فقط «یک Dockerfile از ریشهٔ مخزن» می‌سازد (مثل سرویس داکر پنلی چابکان) | `Dockerfile` ریشه — بخش «استقرار تک‌کانتینری» پایین |

(نسخهٔ اول قدیمی اپ در `Dockerfile.legacy` است و ربطی به استقرار ندارد.)

## استقرار تک‌کانتینری (راه ب — پنل‌های بدون compose)

`Dockerfile` ریشه کل اپ اصلی را در یک ایمیج می‌سازد: nginx (رابط + پروکسی
`/api`) و بک‌اند .NET کنار هم؛ اگر هرکدام بمیرد کانتینر می‌میرد تا پنل
ری‌استارتش کند. در پنل، مخزن را بدهید تا همین Dockerfile ریشه را بسازد و
دامنه را به **پورت ۸۰۸۰** وصل کنید.

متغیرهای محیطی که در پنل بدهید:

| متغیر | چرا |
|---|---|
| `ConnectionStrings__LeaderTrip` | رشتهٔ اتصال PostgreSQL — سرویس آمادهٔ دیتابیس همان هاست را بسازید و رشته‌اش را این‌جا بدهید. **بدونش اپ بالا می‌آید ولی حساب‌های کاربری و ویرایش قیمت‌ها با هر ری‌استارت می‌پرند.** |
| `Api__AdminApiKey` | کلید پنل مدیریت (`openssl rand -base64 32`)؛ خالی = پنل خاموش |
| `Api__AllowedOrigins__0` | آدرس سایت، مثل `https://trip.example.ir` |

اگر پنل اجازهٔ mount دیسک بدهد: `/data/photos` برای عکس‌های چک‌این و
`/tiles` برای نقشهٔ آفلاین ایران (بدونشان اپ کار می‌کند؛ عکس‌ها با
ری‌استارت می‌پرند و نقشه آنلاین می‌ماند).

نمونهٔ ساخت PostgreSQL برای این حالت: در پنل یک سرویس PostgreSQL بسازید و
رشته‌ای شبیه این بدهید (مقدارها را از پنل بردارید):

```
Host=HOST;Port=5432;Database=leadertrip;Username=USER;Password=PASS
```

## استقرار با compose (راه الف — توصیه‌شده)
